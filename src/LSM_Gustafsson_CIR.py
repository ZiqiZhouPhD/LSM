...
def american_option_price(S0, K, T, r, sigma, option_type, num_paths=10000, num_steps=100):
	...
	# Using centered isotonic regression for continuation value estimation
	# Parameters:
	...
	for t_step in range(num_steps - 1, 0, -1):
		...
		if np.sum(in_the_money) > 0:
			...
			fit_diff = centered_isotonic(S_itm, diff, option_type)
			...
			# 	return S_itm, diff, fit_diff, exercise_V, exercise_where

			# Decide whether to exercise or continue
			V[in_the_money] = np.where(fit_diff > 0, exercise_V, continuation_V)
			# V[in_the_money] = np.where(fit_diff > 0, exercise_V, exercise_V - fit_diff)
...
def centered_isotonic(S, V, option_type):
	if option_type.lower() == 'call':
	...
	# Fit
	V_fit = iso_reg.fit_transform(S, V)
	return V_fit + (np.mean(V) - np.mean(V_fit))
