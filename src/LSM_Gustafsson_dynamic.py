...
def american_option_price(S0, K, T, r, sigma, option_type, num_paths=10000, num_steps=100):
	...
	# Use dynamic algorithm to find exercise boundary
	# No regression
	# Parameters:
	...
	for t_step in range(num_steps - 1, 0, -1):
		...
		# Use sorted S
		# Should be faster than sorting S when calling find_exercise_boundary()
		if option_type.lower() == 'call':
			sort_indices = np.argsort(X) # Sort ascending, towards in-the-money
		else:  # put
			sort_indices = np.argsort(-X) # Sort descending, towards in-the-money
		X = X[sort_indices]
		V = V[sort_indices]
		S = S0 * np.exp(X)
		...
		if np.sum(in_the_money) > 0:
			...
			S_boundary = find_exercise_boundary(S_itm, continuation_V, exercise_V)

			# Decide whether to exercise or continue
			if option_type.lower() == 'call':
				V[in_the_money] = np.where(S_itm > S_boundary, exercise_V, continuation_V)
			else:  # put
				V[in_the_money] = np.where(S_itm < S_boundary, exercise_V, continuation_V)
			
			# Record exercise boundary for plotting
			# exercise_S.append(S_boundary)
	...
	if option_type.lower() == 'call':
		...
		V0 = max(V0, K - S0)
	return V0#, times, exercise_S # Uncomment for exercise boundary
...
def find_exercise_boundary(S, cont, exer):
	# S is sorted from OTM to ITM
	diff = exer - cont
	# Find boundary index such that sum(diff[:index]) is minimal
	bound_index = 0
	sum = 0
	min_sum = float('inf')
	for i in range(len(diff)):
		...
		if sum < min_sum:
			min_sum = sum
			bound_index = i
	return S[bound_index] # S[bound_index] is the estimated exercise boundary, itself is not exercised
